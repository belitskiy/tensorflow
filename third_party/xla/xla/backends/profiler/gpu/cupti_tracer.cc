...
static absl::Span<const uint32_t> GetCudaGraphTracingResourceCbids() {
#if CUDA_VERSION >= 11070
  static const uint32_t res_cbids[] = {CUPTI_CBID_RESOURCE_GRAPH_CREATED,
...
absl::Status CuptiTracer::EnableApiTracing() {
  ...
  auto res_cbids = GetCudaGraphTracingResourceCbids();
...
// Resource callback happens longically inside a driver API call's enter/exit.
// Some per-thread data structure to record the graph ids.
absl::Status CuptiTracer::HandleResourceCallback(
    CUpti_CallbackId cbid, const CUpti_CallbackData *cbdata) {
  const CUpti_ResourceData *resource = (CUpti_ResourceData *)cbdata;
  auto *graph_data = (const CUpti_GraphData *)resource->resourceDescriptor;
  GraphResourceCreationInfo &graph_id_info = GetGraphResourceCreationInfo();
  switch (cbid) {
