...
// }
std::optional<PatternInfo> IsDynamicSlicingFusion(
    ...
    if (dynamic_index.has_value() && dynamic_index.value() == 0) {
      ...
      if (Match(instr->fused_instructions_computation()->root_instruction(),
        ...
        if (bitcast_operand == fused_instr) {
          ...
          int64_t num_layers = instr->operand(0)->shape().dimensions(0);
...
// }
std::optional<PatternInfo> IsDynamicUpdateSlicingFusion(
    const UnstackerMetadata& metadata, const HloInstruction* instr,
    int64_t stacked_operand_idx) {
  if (instr->opcode() != HloOpcode::kFusion) {
    return std::nullopt;
...
absl::Status UnstackDynamicUpdateSlicingFusion(
  ...
  std::cout << "FARZIN: " << mutable_dynamic_update_slicing_fusion->ToString()
            << std::endl;
  HloInstruction* new_operand =
      parent_loop->AddInstruction(HloInstruction::CreateCustomCall(
...
std::optional<PatternInfo> GetNestedDynamicSlicingFusion(
    const UnstackerMetadata& metadata, const HloInstruction* instr,
    int64_t stacked_operand_idx) {
  if (instr->opcode() != HloOpcode::kFusion) {
    return std::nullopt;
  ...
  for (HloInstruction* inner_fusion_instr :
    ...
    if (dynamic_index.has_value() && dynamic_index.value() == 0) {
      ...
      int64_t num_layers = inner_fusion_user->operand(0)->shape().dimensions(0);
      PatternInfo pattern_info;
...
std::optional<PatternInfo> IsDSAndDUS(const UnstackerMetadata& metadata,
                                      const HloInstruction* instr,
                                      int64_t stacked_operand_idx) {
  if (instr->opcode() != HloOpcode::kFusion) {
    return std::nullopt;
  ...
  std::optional<PatternInfo> ds_pattern_info =
      IsDynamicSlicingFusion(metadata, instr, instr->operand_index(stacked));
  ...
  VLOG(5) << "FARZIN_FOUND IsDSAndDUS: " << instr->ToString();
  std::cout << "FARZIN_FOUND IsDSAndDUS: " << instr->ToString() << std::endl;
