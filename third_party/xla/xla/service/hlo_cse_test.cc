...
TEST_F(HloCseTest, CombineWithListedOpcodes) {
  const char* const hlo_string = R"(
...
ENTRY %entry (param.1: (f32[], f32[1024,1024], f32[1024,1024])) -> (s32[], f32[1024,1024], f32[1024,1024], f32[1024,1024], f32[1024,1024]) {
  ...
  %zero = s32[] constant(0)
  %broadcast.clone = f32[1024,1024]{1,0} broadcast(f32[] %gte0), dimensions={}, sharding={devices=[2,1,2]0,1,2,3 last_tile_dim_replicate}
  ...
  HloCSE cse(/*is_layout_sensitive=*/false,
             ...
             /*opcodes=*/std::vector({HloOpcode::kBroadcast}));
  TF_ASSERT_OK_AND_ASSIGN(bool result, cse.Run(module.get()));
  VLOG(1) << module->ToString();
  EXPECT_TRUE(result);
}

TEST_F(HloCseTest, DoNotCombineWithNonListedOpcodes) {
  const char* const hlo_string = R"(
HloModule module_entry
