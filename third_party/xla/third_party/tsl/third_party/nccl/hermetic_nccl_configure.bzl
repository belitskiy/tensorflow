...
def _create_hermetic_nccl_repository(repository_ctx):
    nccl_version = repository_ctx.read(repository_ctx.attr.nccl_version)
    config_wrap = {
        "%{nccl_version}": nccl_version,
    }
    repository_ctx.template("BUILD", repository_ctx.attr.system_build_hermetic_tpl, config_wrap)
...
def _create_hermetic_nccl_repository_with_nccl_archive(repository_ctx):
...
def _create_local_nccl_repository(repository_ctx):
    return select({
        "//third_party/gpus/cuda:hermetic_cuda_tools_and_libs": _create_hermetic_nccl_repository(repository_ctx),
        "//conditions:default": _create_hermetic_nccl_repository_with_nccl_archive(repository_ctx),
    })
...
hermetic_nccl_configure = repository_rule(
    ...
    attrs = {
        ...
        "system_build_hermetic_tpl": attr.label(default = Label("//third_party/nccl:system.BUILD.hermetic.tpl")),
    },
)
