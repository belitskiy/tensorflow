...
// This function-
// 1. Get a rougn estimate of the size of the source_module, in bytes.
...
absl::StatusOr<mlir::OwningOpRef<mlir::ModuleOp>> CloneModuleInto(
  ...
  source_module =
      mlir::ModuleOp::create(mlir::UnknownLoc::get(new mlir::MLIRContext()));
...
absl::Status ConvertTFExecutorToTFLOrFlatbuffer(
    ...
    mlir::OwningOpRef<mlir::ModuleOp> module, bool export_to_mlir,
    toco::TocoFlags& toco_flags, const mlir::TFL::PassConfig& pass_config,
    const std::unordered_set<std::string>& saved_model_tags,
  ...
  auto new_context =
      std::make_unique<mlir::MLIRContext>(new_registry, threading);

  TF_ASSIGN_OR_RETURN(
      auto new_module,
      CloneModuleInto(std::move(context), std::move(module), *new_context));

  pass_manager = std::make_unique<mlir::PassManager>(new_context.get());
  mlir::registerPassManagerCLOptions();
  if (mlir::failed(mlir::applyPassManagerCLOptions(*pass_manager))) {
    return absl::InternalError("Failed to apply MLIR pass manager CL options.");
  }
  ...
  status_handler =
      std::make_unique<mlir::StatusScopedDiagnosticHandler>(new_context.get(),
                                                            /*propagate=*/true);
  ...
  if (export_to_mlir) {
    ...
    if (failed(pass_manager->run(new_module.get()))) {
      return status_handler->ConsumeStatus();
    }
  ...
  if (!tflite::MlirToFlatBufferTranslateFunction(new_module.get(), options,
  ...
  if (mlir::failed(new_module->verifyInvariants())) {
    return status_handler->Combine(
        absl::InternalError("Final module is invalid."));
  }

  // Need to explicitly reset the pass manager and status handler because the
  // destruction order of the unique_ptrs is not deterministic. We need to reset
  // the pass manager and status handler before the new context is destroyed.
  pass_manager.reset();
  status_handler.reset();
